...
func TestPipeline_TaskLookup(t *testing.T) {
	...
	tests := []struct {
		...
		{"should return specific task with known task and valid project alias", args{name: "task", project: "int"}, &internalTask, false},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := pipeline.TaskLookup(tt.args.name, tt.args.project)
